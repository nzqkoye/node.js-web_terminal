...
upxray(){
url="https://github.com/yonggekkk/argosbx/releases/download/argosbx/xray-$cpu"; out="/app/s/xray"; (command -v curl >/dev/null 2>&1 && curl -Lo "$out" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -O "$out" --tries=2 "$url")
chmod +x "/app/s/xray"
sbcore=$("/app/s/xray" version 2>/dev/null | awk '/^Xray/{print $2}')
echo "已安装Xray正式版内核：$sbcore"
}
upsingbox(){
url="https://github.com/yonggekkk/argosbx/releases/download/argosbx/sing-box-$cpu"; out="/app/s/sing-box"; (command -v curl>/dev/null 2>&1 && curl -Lo "$out" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -O "$out" --tries=2 "$url")
chmod +x "/app/s/sing-box"
sbcore=$("/app/s/sing-box" version 2>/dev/null | awk '/version/{print $NF}')
echo "已安装Sing-box正式版内核：$sbcore"
}
insuuid(){
if [ -z "$uuid" ] && [ ! -e "/app/s/uuid" ]; then
if [ -e "/app/s/sing-box" ]; then
uuid=$("/app/s/sing-box" generate uuid)
else
uuid=$("/app/s/xray" uuid)
fi
echo "$uuid" > "/app/s/uuid"
elif [ -n "$uuid" ]; then
echo "$uuid" > "/app/s/uuid"
fi
uuid=$(cat "/app/s/uuid")
...
echo "=========启用xray内核========="
mkdir -p "/app/s/xrk"
if [ ! -e "/app/s/xray" ]; then
upxray
fi
cat > "/app/s/xr.json" <<EOF
{
  "log": {
  "loglevel": "none"
  },
...
echo "$ym_vl_re" > "/app/s/ym_vl_re"
echo "Reality域名：$ym_vl_re"
if [ ! -e "/app/s/xrk/private_key" ]; then
key_pair=$("/app/s/xray" x25519)
private_key=$(echo "$key_pair" | awk -F':' '/PrivateKey/ {print $2}' | xargs)
public_key=$(echo "$key_pair" | awk -F':' '/Password/ {print $2}' | xargs)
short_id=$(date +%s%N | sha256sum | cut -c 1-8)
echo "$private_key" > "/app/s/xrk/private_key"
echo "$public_key" > "/app/s/xrk/public_key"
echo "$short_id" > "/app/s/xrk/short_id"
fi
private_key_x=$(cat "/app/s/xrk/private_key")
public_key_x=$(cat "/app/s/xrk/public_key")
short_id_x=$(cat "/app/s/xrk/short_id")
fi
if [ -n "$xhp" ] || [ -n "$vxp" ] || [ -n "$vwp" ]; then
if [ ! -e "/app/s/xrk/dekey" ]; then
vlkey=$("/app/s/xray" vlessenc)
dekey=$(echo "$vlkey" | grep '"decryption":' | sed -n '2p' | cut -d' ' -f2- | tr -d '"')
enkey=$(echo "$vlkey" | grep '"encryption":' | sed -n '2p' | cut -d' ' -f2- | tr -d '"')
echo "$dekey" > "/app/s/xrk/dekey"
echo "$enkey" > "/app/s/xrk/enkey"
fi
dekey=$(cat "/app/s/xrk/dekey")
enkey=$(cat "/app/s/xrk/enkey")
fi

if [ -n "$xhp" ]; then
xhp=xhpt
if [ -z "$port_xh" ] && [ ! -e "/app/s/port_xh" ]; then
port_xh=$(shuf -i 10000-65535 -n 1)
echo "$port_xh" > "/app/s/port_xh"
elif [ -n "$port_xh" ]; then
echo "$port_xh" > "/app/s/port_xh"
fi
port_xh=$(cat "/app/s/port_xh")
...
if [ -n "$vxp" ]; then
vxp=vxpt
if [ -z "$port_vx" ] && [ ! -e "/app/s/port_vx" ]; then
port_vx=$(shuf -i 10000-65535 -n 1)
echo "$port_vx" > "/app/s/port_vx"
elif [ -n "$port_vx" ]; then
echo "$port_vx" > "/app/s/port_vx"
fi
port_vx=$(cat "/app/s/port_vx")
echo "Vless-xhttp-enc端口：$port_vx"
if [ -n "$cdnym" ]; then
echo "$cdnym" > "/app/s/cdnym"
...
if [ -n "$vwp" ]; then
vwp=vwpt
if [ -z "$port_vw" ] && [ ! -e "/app/s/port_vw" ]; then
port_vw=$(shuf -i 10000-65535 -n 1)
echo "$port_vw" > "/app/s/port_vw"
elif [ -n "$port_vw" ]; then
echo "$port_vw" > "/app/s/port_vw"
fi
port_vw=$(cat "/app/s/port_vw")
...
if [ -n "$vlp" ]; then
vlp=vlpt
if [ -z "$port_vl_re" ] && [ ! -e "/app/s/port_vl_re" ]; then
port_vl_re=$(shuf -i 10000-65535 -n 1)
echo "$port_vl_re" > "/app/s/port_vl_re"
elif [ -n "$port_vl_re" ]; then
echo "$port_vl_re" > "/app/s/port_vl_re"
fi
port_vl_re=$(cat "/app/s/port_vl_re")
...
EOF
insuuid
if [ ! -f "/app/s/SHA256.txt" ]; then
command -v openssl >/dev/null 2>&1 && openssl ecparam -genkey -name prime256v1 -out "/app/s/private.key" >/dev/null 2>&1
command -v openssl >/dev/null 2>&1 && openssl req -new -x509 -days 36500 -key "/app/s/private.key" -out "/app/s/cert.crt" -subj "/CN=www.bing.com" >/dev/null 2>&1
#if [ ! -f "/app/s/private.key" ]; then
#url="https://github.com/yonggekkk/argosbx/releases/download/argosbx/private.key"; out="/app/s/private.key"; (command -v curl>/dev/null 2>&1 && curl -Ls -o "$out" --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -q -O "$out" --tries=2 "$url")
#url="https://github.com/yonggekkk/argosbx/releases/download/argosbx/cert.crt"; out="/app/s/cert.crt"; (command -v curl>/dev/null 2>&1 && curl -Ls -o "$out" --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -q -O "$out" --tries=2 "$url")
#echo "fc6dca8cfc4081102aa9655d0d4805c27d7266f605541d242ad66ad00a284a35" > "/app/s/SHA256.txt"
#else
SHA256=$(openssl x509 -in /app/s/cert.crt -outform DER | sha256sum | awk '{print $1}')
echo "$SHA256" > "/app/s/SHA256.txt"
#fi
fi
if [ -n "$hyp" ]; then
hyp=hypt
if [ -z "$port_hy2" ] && [ ! -e "/app/s/port_hy2" ]; then
port_hy2=$(shuf -i 10000-65535 -n 1)
echo "$port_hy2" > "/app/s/port_hy2"
elif [ -n "$port_hy2" ]; then
echo "$port_hy2" > "/app/s/port_hy2"
fi
port_hy2=$(cat "/app/s/port_hy2")
...
cat >> "/app/s/sb.json" <<EOF
    {
        ...
        "tls": {
            ...
            "certificate_path": "/app/s/cert.crt",
            "key_path": "/app/s/private.key"
        }
    },
EOF
else
...
if [ -n "$tup" ]; then
tup=tupt
if [ -z "$port_tu" ] && [ ! -e "/app/s/port_tu" ]; then
port_tu=$(shuf -i 10000-65535 -n 1)
echo "$port_tu" > "/app/s/port_tu"
elif [ -n "$port_tu" ]; then
echo "$port_tu" > "/app/s/port_tu"
fi
port_tu=$(cat "/app/s/port_tu")
echo "Tuic端口：$port_tu"
cat >> "/app/s/sb.json" <<EOF
        {
            "type":"tuic",
...
if [ -n "$anp" ]; then
anp=anpt
if [ -z "$port_an" ] && [ ! -e "/app/s/port_an" ]; then
port_an=$(shuf -i 10000-65535 -n 1)
echo "$port_an" > "/app/s/port_an"
elif [ -n "$port_an" ]; then
echo "$port_an" > "/app/s/port_an"
fi
port_an=$(cat "/app/s/port_an")
...
echo "Reality域名：$ym_vl_re"
mkdir -p "/app/s/sbk"
if [ ! -e "/app/s/sbk/private_key" ]; then
key_pair=$("/app/s/sing-box" generate reality-keypair)
private_key=$(echo "$key_pair" | awk '/PrivateKey/ {print $2}' | tr -d '"')
public_key=$(echo "$key_pair" | awk '/PublicKey/ {print $2}' | tr -d '"')
short_id=$("/app/s/sing-box" generate rand --hex 4)
echo "$private_key" > "/app/s/sbk/private_key"
echo "$public_key" > "/app/s/sbk/public_key"
echo "$short_id" > "/app/s/sbk/short_id"
fi
private_key_s=$(cat "/app/s/sbk/private_key")
public_key_s=$(cat "/app/s/sbk/public_key")
short_id_s=$(cat "/app/s/sbk/short_id")
if [ -z "$port_ar" ] && [ ! -e "/app/s/port_ar" ]; then
port_ar=$(shuf -i 10000-65535 -n 1)
echo "$port_ar" > "/app/s/port_ar"
elif [ -n "$port_ar" ]; then
echo "$port_ar" > "/app/s/port_ar"
fi
port_ar=$(cat "/app/s/port_ar")
...
if [ -n "$ssp" ]; then
ssp=sspt
if [ ! -e "/app/s/sskey" ]; then
sskey=$("/app/s/sing-box" generate rand 16 --base64)
echo "$sskey" > "/app/s/sskey"
fi
if [ -z "$port_ss" ] && [ ! -e "/app/s/port_ss" ]; then
port_ss=$(shuf -i 10000-65535 -n 1)
echo "$port_ss" > "/app/s/port_ss"
elif [ -n "$port_ss" ]; then
echo "$port_ss" > "/app/s/port_ss"
fi
sskey=$(cat "/app/s/sskey")
port_ss=$(cat "/app/s/port_ss")
...
if [ -n "$vmp" ]; then
vmp=vmpt
if [ -z "$port_vm_ws" ] && [ ! -e "/app/s/port_vm_ws" ]; then
port_vm_ws=$(shuf -i 10000-65535 -n 1)
echo "$port_vm_ws" > "/app/s/port_vm_ws"
elif [ -n "$port_vm_ws" ]; then
echo "$port_vm_ws" > "/app/s/port_vm_ws"
fi
port_vm_ws=$(cat "/app/s/port_vm_ws")
echo "Vmess-ws端口：$port_vm_ws"
if [ -n "$cdnym" ]; then
echo "$cdnym" > "/app/s/cdnym"
echo "80系CDN或者回源CDN的host域名 (确保IP已解析在CF域名)：$cdnym"
fi
if [ -e "/app/s/xr.json" ]; then
cat >> "/app/s/xr.json" <<EOF
        {
            "tag": "vmess-xr",
            "listen": "::",
...
if [ -n "$sop" ]; then
sop=sopt
if [ -z "$port_so" ] && [ ! -e "/app/s/port_so" ]; then
port_so=$(shuf -i 10000-65535 -n 1)
echo "$port_so" > "/app/s/port_so"
elif [ -n "$port_so" ]; then
echo "$port_so" > "/app/s/port_so"
fi
port_so=$(cat "/app/s/port_so")
...
NoNewPrivileges=yes
TimeoutStartSec=0
ExecStart=/app/s/xray run -c /app/s/xr.json
Restart=on-failure
RestartSec=5s
...
description="xr service"
command="/app/s/xray"
command_args="run -c /app/s/xr.json"
...
nohup "/app/s/xray" run -c "/app/s/xr.json" >/dev/null 2>&1 &
fi
fi
if [ -e "/app/s/sb.json" ]; then
sed -i '${s/,\s*$//}' "/app/s/sb.json"
cat >> "/app/s/sb.json" <<EOF
  ],
  "outbounds": [
    {
...
NoNewPrivileges=yes
TimeoutStartSec=0
ExecStart=/app/s/sing-box run -c /app/s/sb.json
Restart=on-failure
RestartSec=5s
StandardOutput=journal
StandardError=journal
...
#!/sbin/openrc-run
description="sb service"
command="/app/s/sing-box"
command_args="run -c /app/s/sb.json"
command_background=yes
...
nohup "/app/s/sing-box" run -c "/app/s/sb.json" >/dev/null 2>&1 &
fi
fi
}
ins(){
...
url="https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-$cpu"; out="/app/s/cloudflared"; (command -v curl>/dev/null 2>&1 && curl -Lo "$out" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -O "$out" --tries=2 "$url")
chmod +x "/app/s/cloudflared"
fi
if [ "$argo" = "vmpt" ]; then argoport=$(cat "/app/s/port_vm_ws" 2>/dev/null); echo "Vmess" > "/app/s/vlvm"; elif [ "$argo" = "vwpt" ]; then argoport=$(cat "/app/s/port_vw" 2>/dev/null); echo "Vless" > "/app/s/vlvm"; fi; echo "$argoport" > "/app/s/argoport.log"
...
NoNewPrivileges=yes
TimeoutStartSec=0
ExecStart=/app/s/cloudflared tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token "${ARGO_AUTH}"
Restart=on-failure
RestartSec=5s
[Install]
WantedBy=multi-user.target
...
#!/sbin/openrc-run
description="argo service"
command="/app/s/cloudflared tunnel"
...
nohup "/app/s/cloudflared" tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token "${ARGO_AUTH}" >/dev/null 2>&1 &
fi
echo "${ARGO_DOMAIN}" > "/app/s/sbargoym.log"
echo "${ARGO_AUTH}" > "/app/s/sbargotoken.log"
else
argoname='临时'
nohup "/app/s/cloudflared" tunnel --url http://localhost:$(cat /app/s/argoport.log) --edge-ip-version auto --no-autoupdate --protocol http2 > /app/s/argo.log 2>&1 &
fi
echo "申请Argo$argoname隧道中……请稍等"
sleep 15
if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
argodomain=$(cat "/app/s/sbargoym.log" 2>/dev/null)
else
argodomain=$(grep -a trycloudflare.com "/app/s/argo.log" 2>/dev/null | awk 'NR==2{print}' | awk -F// '{print $2}' | awk '{print $1}')
fi
...
echo '@reboot sleep 10 && /bin/sh -c "nohup /app/s/sing-box run -c /app/s/sb.json >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
fi
...
echo '@reboot sleep 10 && /bin/sh -c "nohup /app/s/xray run -c /app/s/xr.json >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
fi
fi
...
if [ -n "${ARGO_DOMAIN}" ] && [ -n "${ARGO_AUTH}" ]; then
if ! pidof systemd >/dev/null 2>&1 && ! command -v rc-service >/dev/null 2>&1; then
echo '@reboot sleep 10 && /bin/sh -c "nohup /app/s/cloudflared tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token $(cat /app/s/sbargotoken.log 2>/dev/null) >/dev/null 2>&1 &"' >> /tmp/crontab.tmp
fi
else
if command -v apk >/dev/null 2>&1; then
...
nohup /app/s/cloudflared tunnel --url http://localhost:\$(cat /app/s/argoport.log) --edge-ip-version auto --no-autoupdate --protocol http2 > /app/s/argo.log 2>&1 &
sleep 10
HOME="/usr/lib/apt" /app/b/agsbx list >/dev/null 2>&1
EOF
chmod +x /etc/local.d/alpineargosbx.start
rc-update add local default >/dev/null 2>&1
else
echo '@reboot sleep 10 && /bin/bash -c "nohup /app/s/cloudflared tunnel --url http://localhost:$(cat /app/s/argoport.log) --edge-ip-version auto --no-autoupdate --protocol http2 > /app/s/argo.log 2>&1 & sleep 10 && bash /app/b/agsbx list >/dev/null 2>&1"' >> /tmp/crontab.tmp
fi
fi
fi
...
cdnip1="$1"
cdnip2="$2"
echo "$cdnip1" > "/app/s/cdnip1"
echo "$cdnip2" > "/app/s/cdnip2"
else
if [ -f "/app/s/cdnip1" ] && [ -f "/app/s/cdnip2" ]; then
cdnip1=$(cat "/app/s/cdnip1")
cdnip2=$(cat "/app/s/cdnip2")
else
...
echo "Sing-box (版本V$("/app/s/sing-box" version 2>/dev/null | awk '/version/{print $NF}'))：运行中"
else
echo "Sing-box：未启用"
fi
if echo "$procs" | grep -Eq 'agsbx/x' || pgrep -f 'agsbx/x' >/dev/null 2>&1; then
echo "Xray (版本V$("/app/s/xray" version 2>/dev/null | awk '/^Xray/{print $2}'))：运行中"
else
echo "Xray：未启用"
fi
if echo "$procs" | grep -Eq 'agsbx/c' || pgrep -f 'agsbx/c' >/dev/null 2>&1; then
echo "Argo (版本V$("/app/s/cloudflared" version 2>/dev/null | awk '{print $3}'))：运行中"
else
echo "Argo：未启用"
fi
}
...
server_ip="[$serip]"
echo "$server_ip" > "/app/s/server_ip.log"
else
server_ip="$serip"
echo "$server_ip" > "/app/s/server_ip.log"
fi
...
ipchange
rm -rf "/app/s/jhsub.txt"
uuid=$(cat "/app/s/uuid")
server_ip=$(cat "/app/s/server_ip.log")
sxname=$(cat "/app/s/name" 2>/dev/null)
xvvmcdnym=$(cat "/app/s/cdnym" 2>/dev/null)
echo "*********************************************************"
echo "*********************************************************"
...
echo
ym_vl_re=$(cat "/app/s/ym_vl_re" 2>/dev/null)
cfipsj() { echo $((RANDOM % 13 + 1)); }
if [ -e "/app/s/xray" ]; then
private_key_x=$(cat "/app/s/xrk/private_key" 2>/dev/null)
public_key_x=$(cat "/app/s/xrk/public_key" 2>/dev/null)
short_id_x=$(cat "/app/s/xrk/short_id" 2>/dev/null)
enkey=$(cat "/app/s/xrk/enkey" 2>/dev/null)
fi
if [ -e "/app/s/sing-box" ]; then
private_key_s=$(cat "/app/s/sbk/private_key" 2>/dev/null)
public_key_s=$(cat "/app/s/sbk/public_key" 2>/dev/null)
short_id_s=$(cat "/app/s/sbk/short_id" 2>/dev/null)
sskey=$(cat "/app/s/sskey" 2>/dev/null)
fi
if grep xhttp-reality "/app/s/xr.json" >/dev/null 2>&1; then
...
echo "$vl_xh_link" >> "/app/s/jhsub.txt"
echo "$vl_xh_link"
echo
fi
if grep vless-xhttp "/app/s/xr.json" >/dev/null 2>&1; then
...
echo "$vl_vx_link" >> "/app/s/jhsub.txt"
echo "$vl_vx_link"
echo
if [ -f "/app/s/cdnym" ]; then
...
echo "$vl_vx_cdn_link" >> "/app/s/jhsub.txt"
echo "$vl_vx_cdn_link"
echo
fi
fi
if grep vless-ws "/app/s/xr.json" >/dev/null 2>&1; then
...
echo "$vl_vw_link" >> "/app/s/jhsub.txt"
echo "$vl_vw_link"
echo
if [ -f "/app/s/cdnym" ]; then
...
echo "$vl_vw_cdn_link" >> "/app/s/jhsub.txt"
echo "$vl_vw_cdn_link"
echo
fi
fi
if grep reality-vision "/app/s/xr.json" >/dev/null 2>&1; then
...
if grep ss-2022 "/app/s/sb.json" >/dev/null 2>&1; then
...
if grep vmess-xr "/app/s/xr.json" >/dev/null 2>&1 || grep vmess-sb "/app/s/sb.json" >/dev/null 2>&1; then
...
echo "$vm_cdn_link" >> "/app/s/jhsub.txt"
echo "$vm_cdn_link"
echo
fi
fi
if grep anytls-sb "/app/s/sb.json" >/dev/null 2>&1; then
echo "💣【 AnyTLS 】节点信息如下："
port_an=$(cat "/app/s/port_an")
an_link="anytls://$uuid@$server_ip:$port_an?insecure=1&allowInsecure=1#${sxname}anytls-$hostname"
echo "$an_link" >> "/app/s/jhsub.txt"
echo "$an_link"
echo
sbanpt(){
cat <<EOF
...
if grep anyreality-sb "/app/s/sb.json" >/dev/null 2>&1; then
...
if grep hy2-sb "/app/s/sb.json" >/dev/null 2>&1; then
echo "💣【 Hysteria2 】节点信息如下："
SHA256=$(cat "/app/s/SHA256.txt")
port_hy2=$(cat "/app/s/port_hy2")
...
if grep tuic5-sb "/app/s/sb.json" >/dev/null 2>&1; then
echo "💣【 Tuic 】节点信息如下："
port_tu=$(cat "/app/s/port_tu")
...
echo "$tuic5_link" >> "/app/s/jhsub.txt"
echo "$tuic5_link"
echo
sbtupt(){
cat <<EOF
...
if grep socks5-xr "/app/s/xr.json" >/dev/null 2>&1 || grep socks5-sb "/app/s/sb.json" >/dev/null 2>&1; then
echo "💣【 Socks5 】客户端信息如下："
port_so=$(cat "/app/s/port_so")
...
argodomain=$(cat "/app/s/sbargoym.log" 2>/dev/null)
[ -z "$argodomain" ] && argodomain=$(grep -a trycloudflare.com "/app/s/argo.log" 2>/dev/null | awk 'NR==2{print}' | awk -F// '{print $2}' | awk '{print $1}')
if [ -n "$argodomain" ]; then
vlvm=$(cat /app/s/vlvm 2>/dev/null)
if [ "$vlvm" = "Vmess" ]; then
...
echo "$vmatls_link1" >> "/app/s/jhsub.txt"
...
echo "$vmatls_link2" >> "/app/s/jhsub.txt"
...
echo "$vmatls_link3" >> "/app/s/jhsub.txt"
...
echo "$vmatls_link4" >> "/app/s/jhsub.txt"
...
echo "$vmatls_link5" >> "/app/s/jhsub.txt"
...
echo "$vmatls_link6" >> "/app/s/jhsub.txt"
...
echo "$vma_link7" >> "/app/s/jhsub.txt"
...
echo "$vma_link8" >> "/app/s/jhsub.txt"
...
echo "$vma_link9" >> "/app/s/jhsub.txt"
...
echo "$vma_link10" >> "/app/s/jhsub.txt"
...
echo "$vma_link11" >> "/app/s/jhsub.txt"
...
echo "$vma_link12" >> "/app/s/jhsub.txt"
...
echo "$vma_link13" >> "/app/s/jhsub.txt"
sbvmargopt(){
cat <<EOF
{
            "server": "$cdnip1",
...
echo "$vwatls_link1" >> "/app/s/jhsub.txt"
...
echo "$vwa_link2" >> "/app/s/jhsub.txt"
fi
sbtk=$(cat "/app/s/sbargotoken.log" 2>/dev/null)
if [ -n "$sbtk" ]; then
nametn="Argo固定隧道token：$sbtk"
fi
argoshow=$(
echo "Argo隧道端口正在使用$vlvm-ws主协议端口：$(cat /app/s/argoport.log 2>/dev/null)
Argo域名：$argodomain
$nametn
...
echo "$argoshow"
echo
if [ -s /app/s/subport.log ]; then
showsubport=$(cat /app/s/subport.log)
if ps -ef 2>/dev/null | grep "$showsubport" | grep -v grep >/dev/null; then
showsubtoken=$(cat /app/s/subtoken.log 2>/dev/null)
subip=$(cat /app/s/server_ip.log 2>/dev/null)
...
echo
echo "---------------------------------------------------------"
echo "聚合节点信息，请进入 /app/s/jhsub.txt 文件目录查看或者运行 cat /app/s/jhsub.txt 查看"
echo "========================================================="
...
if [ "$1" = "del" ]; then
cleandel
rm -rf sbx_update "/app/s" "/usr/lib/apt/websbx"
echo "卸载完成"
...
cleandel
rm -rf "/app/s"/{sb.json,xr.json,sbargoym.log,sbargotoken.log,argo.log,argoport.log,cdnym,name}
...
if [ -e "/app/s/sbargotoken.log" ]; then
if pidof systemd >/dev/null 2>&1; then
systemctl restart argo >/dev/null 2>&1
elif command -v rc-service >/dev/null 2>&1; then
rc-service argo restart >/dev/null 2>&1
else
nohup /app/s/cloudflared tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token $(cat /app/s/sbargotoken.log 2>/dev/null) >/dev/null 2>&1 &
fi
else
nohup /app/s/cloudflared tunnel --url http://localhost:$(cat /app/s/argoport.log 2>/dev/null) --edge-ip-version auto --no-autoupdate --protocol http2 > /app/s/argo.log 2>&1 &
fi
;;
esac
done
...
subtokenipsub(){
if [ -z "$subid" ]; then
subtoken="$(cat "/app/s/uuid")"
else
subtoken="$subid"
fi
rm -rf /usr/lib/apt/websbx/"$(cat /app/s/subtoken.log 2>/dev/null)"
echo $subtoken > /app/s/subtoken.log
}
subportipsub(){
if [ -z "$subpt" ]; then
if [ -n "$(cat "/app/s/subport.log" 2>/dev/null)" ]; then
subport=$(cat /app/s/subport.log)
else
...
echo $subport > /app/s/subport.log
}
subtokenipsub && subportipsub
echo "请稍后…………"
kill -15 $(pgrep -f 'websbx' 2>/dev/null) >/dev/null 2>&1
mkdir -p /usr/lib/apt/websbx/"$(cat /app/s/subtoken.log 2>/dev/null)"
ln -sf /app/s/clmi.yaml /usr/lib/apt/websbx/"$(cat /app/s/subtoken.log 2>/dev/null)"/clmi.yaml
ln -sf /app/s/sbox.json /usr/lib/apt/websbx/"$(cat /app/s/subtoken.log 2>/dev/null)"/sbox.json
ln -sf /app/s/jhsub.txt /usr/lib/apt/websbx/"$(cat /app/s/subtoken.log 2>/dev/null)"/jhsub.txt
if command -v apk >/dev/null 2>&1; then
busybox-extras httpd -f -p "$(cat /app/s/subport.log 2>/dev/null)" -h /usr/lib/apt/websbx > /dev/null 2>&1 &
else
busybox httpd -f -p "$(cat /app/s/subport.log 2>/dev/null)" -h /usr/lib/apt/websbx > /dev/null 2>&1 &
fi
sleep 5
if command -v apk >/dev/null 2>&1; then
...
sed -i '/websbx/d' /tmp/crontab.tmp
echo '@reboot sleep 10 && /bin/bash -c "busybox httpd -f -p $(cat /app/s/subport.log 2>/dev/null) -h /usr/lib/apt/websbx > /dev/null 2>&1 &"' >> /tmp/crontab.tmp
crontab /tmp/crontab.tmp >/dev/null 2>&1
rm /tmp/crontab.tmp
fi
echo "本地IP订阅链接已更新完成"
